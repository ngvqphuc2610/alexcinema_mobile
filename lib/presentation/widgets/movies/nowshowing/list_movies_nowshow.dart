...
class NowShowingListPage extends StatelessWidget {
  const NowShowingListPage({super.key});
      ...
      create: (_) => serviceLocator<MovieBloc>()
        ..add(
          const MoviesRequested(
            query: MovieQueryDto(status: 'now showing', limit: 100),
          ),
        ),
      child: const _NowShowingListView(),
    );
  }
}

class _NowShowingListView extends StatelessWidget {
  const _NowShowingListView();

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Phim dang chieu'),
      ),
      body: BlocBuilder<MovieBloc, MoviesState>(
        builder: (context, state) {
          ...
          if (state.items.isEmpty) {
            return const Center(child: Text('Chua co phim dang chieu'));
          }
